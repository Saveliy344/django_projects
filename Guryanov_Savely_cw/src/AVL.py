...
class AVLTree:
    ...
    def sort(self, tree, node):
        if node is not None:
            self.sort(tree, node.left_child)
            self.list_tree.append(node.value)
            self.sort(tree, node.right_child)
...
if __name__ == '__main__':
    tree = AVLTree()
    for i in [randint(1, 100) for j in range(100)]:
        tree.insert(i)
    tree.sort(tree, tree.root)
